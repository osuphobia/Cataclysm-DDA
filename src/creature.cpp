...
projectile_attack_results Creature::select_body_part_projectile_attack(
    const projectile &proj, const bool magic, const double missed_by,
    const weakpoint_attack &attack ) const
{
    ...
    const monster *mon = as_monster();
    if( mon ) {
        if( attack.accuracy < accuracy_standard ) {
            ret.wp = *mon->type->weakpoints.select_weakpoint( attack );
        }
        if( magic ) {
            // do nothing special, no damage mults, nothing
        } else if( !has_flag( mon_flag_NOHEAD) && attack.accuracy < accuracy_headshot &&
                   ret.wp.is_head && ret.max_damage * crit_multiplier > get_hp_max() ) {
            ret.message = _( "Headshot!!" );
            ret.gmtSCTcolor = m_headshot;
            ret.damage_mult *= crit_multiplier * rng_float( 0.95, 1.05 );
            ret.is_crit = true;
            ret.is_headshot = true;
        } else if( attack.accuracy < accuracy_critical && ret.wp.is_good &&
                   ret.max_damage * crit_multiplier * 2 > get_hp_max() ) {
            ret.message = _( "Critical!!" );
            ret.gmtSCTcolor = m_critical;
            ret.damage_mult *= crit_multiplier * rng_float( 0.75, 1.0 );
            ret.is_crit = true;
        } else if( attack.accuracy < accuracy_standard ) {
            ret.message = _( "Good hit!" );
            ret.gmtSCTcolor = m_good;
            ret.damage_mult *= std_hit_mult * rng_float( 0.5, 0.75 );
        } else if( attack.accuracy < accuracy_grazing ) {
    ...
    if( magic ) {
        // do nothing special, no damage mults, nothing
    } else if( attack.accuracy < accuracy_headshot && ret.bp_hit == body_part_head &&
        ...
        ret.is_crit = true;
        ret.is_headshot = true;
    } else if( attack.accuracy < accuracy_critical &&
               ret.max_damage * crit_multiplier > get_hp_max( ret.bp_hit ) ) {
        ret.message = _( "Critical!" );
        ret.gmtSCTcolor = m_critical;
        ...
        ret.damage_mult *= std_hit_mult + ( crit_multiplier - std_hit_mult ) * crit_mod;
        ret.is_crit = true;
    } else if( attack.accuracy < accuracy_goodhit ) {
        ret.message = _( "Good hit!" );
        ret.gmtSCTcolor = m_good;
                                       ...
                                       bool print_messages, const weakpoint_attack &wp_attack )
{
    ...
    projectile_attack_results hit_selection = select_body_part_projectile_attack( proj, magic,
            missed_by, wp_attack_copy );
